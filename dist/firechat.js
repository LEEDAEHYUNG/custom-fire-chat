...
(function() {
  ...
  Firechat.prototype = {
    ...
    _loadInitUserMetadata: function(onComplete) {
      var self = this;

      // Update the user record with a default name on user's first visit.
      this._userRef.transaction(function(current) {
        if (!current || !current.id || !current.name || !current.userInfo) {
          return {
            id: self._userId,
            name: self._userName,
  ...
  Firechat.prototype.initUser = function(userId, userName, userInfo, callback) {
    var self = this;
    ...
    self._firebaseApp.auth().onAuthStateChanged(function(user) {
      if (user) {
        ...
        self._loadInitUserMetadata(function() {
          root.setTimeout(function() {
            callback();
          }, 0);
        });
